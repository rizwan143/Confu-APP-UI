...
open class FloatingTabBar: UIView {
    ...
    //Default Value 20
    open var bottomRadius: CGFloat = 2 {
        didSet { setNeedsLayout() }
    }
    ...
    func updateImage(index: Int, image: UIImage, selectedImage : UIImage){
        if items.count > index{
           // items[index] = FloatingTabItem(image: image)
            items[index] = FloatingTabItem(selectedImage: selectedImage, normalImage: image)
        }
    }
    ...
    func createPath() {
        ...
        let circleSide: CGFloat = bounds.height - (bottomMargin + spacing + 15)
        
        let background = UIBezierPath()
//        background.bounds.topLeft = 20.0
//        background.bounds.topRight = 20.0
    ...
    func layoutIcons() {
        ...
        let rect = bounds
        let circleSide: CGFloat = rect.height - (bottomMargin + spacing + 15)
        print(circleSide)
        let circleV = CGPoint(x: 0, y: circleSide / 2)
